...
@Repository
public class RecordOneDao extends HibernateDao {

	@Autowired
	public RecordOneDao(SessionFactory sessionFactory) {
			...
			while (it.hasNext()) {
				...
				if (!value.equals("")) {
					if (key.equals("partString")) {
						criteria.add(Restrictions.like("partString", (String) value, MatchMode.ANYWHERE));
					} else {
