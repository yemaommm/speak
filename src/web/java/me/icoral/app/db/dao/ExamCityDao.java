...
@Repository
public class ExamCityDao extends HibernateDao {

	@Autowired
	public ExamCityDao(SessionFactory sessionFactory) {
	...
	protected void addRestrictions(Criteria criteria, JSONObject q) {
		if (q.containsKey("$criteria")) {
			...
			while (it.hasNext()) {
				...
				if (!value.equals("")) {
					if (key.equals("examCity")) {
						criteria.add(Restrictions.like("examCity", (String) value, MatchMode.ANYWHERE));
					} else {
